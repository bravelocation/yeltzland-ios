...
class TVDataSource: NSObject, UITableViewDataSource {
    ...
    private var lastGames = Array<TVDataItem>()
    private var nextGames = Array<TVDataItem>()
    private var currentScores = Array<TVDataItem>()
    ...
    func loadLatestData() {
        ...
        if (opponent.count > 0) {
            ...
            var resultColor = AppColors.TVText
            ...
            self.lastGames.append(TVDataItem(opponent: self.gameSettings.displayLastOpponent, scoreOrDate: self.gameSettings.lastScore, color:resultColor))
        }
        
        // Get current score if applicable
        self.currentScores.removeAll()
        if (self.gameSettings.gameScoreForCurrentGame) {
            self.currentScores.append(TVDataItem(opponent: self.gameSettings.displayNextOpponent, scoreOrDate: self.gameSettings.currentScore))
        }
        ...
        for fixture in FixtureManager.instance.GetNextFixtures(fixturesNeeded) {
            ...
            if (i > 0 || self.currentScores.count == 0) {
                let fixtureData = TVDataItem(opponent: fixture.displayOpponent, scoreOrDate: fixture.fullDisplayKickoffTime)
    ...
    private func dataItemsForSection(section:Int) -> Array<TVDataItem>? {
        var firstSection:Array<TVDataItem>? = nil
        var secondSection:Array<TVDataItem>? = nil
        var thirdSection:Array<TVDataItem>? = nil
    ...
    func tableView(_ tableView: UITableView,
        ...
        cell.backgroundColor = AppColors.TVBackground
        ...
        cell.textLabel?.textColor = AppColors.TVText
